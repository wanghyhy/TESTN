...
def main(args):
    data = LocationRelDataset(name=args.dataset,raw_dir=args.data_dir)
    ...
    log = str(args)+'\n'
    # create model
    model = TESTN(num_nodes, args.n_hidden, num_rels,
    ...
    hop2_dict,mid_dict,time_relation = utils.build_hop2_dict(train_data, relation_dict, rel_list, time_list, args.dataset, path_len=2)
    ...
    while True:
        model.train()
        epoch += 1
        tri_pos, tri_neg, dir_pair_data, pair_label = utils.generate_triplet(mid_dict, num_nodes,time_list, negative_rate=1)
        ...
        loss,predict_tloss,tloss3,tpair_loss = model.get_loss(g, embed, data, labels,tri_pos, tri_neg, dir_pair_data, pair_label,args.ssl_weight)
        loss.backward()
        optimizer.step()
        train_loss = loss.item()
        predict_tloss,tloss3,tpair_loss=predict_tloss.item(),tloss3.item(),tpair_loss.item()
        print("Epoch {:04d} | Loss {:.4f} predict_tloss {:.4f} tloss3 {:.4f} tpair_loss {:.4f} | Best Valid MRR@10 {:.4f}".format(epoch, train_loss,predict_tloss,tloss3,tpair_loss, best_mrr))
        log += "Epoch {:04d} | Loss {:.4f} predict_tloss {:.4f} tloss3 {:.4f} tpair_loss {:.4f} | Best Valid MRR@10 {:.4f}\n".format(epoch, train_loss,predict_tloss,tloss3,tpair_loss, best_mrr)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--dataset", type=str, default='chicago')
    ...
    parser.add_argument("--ssl_weight", type=float, default=2.0)
