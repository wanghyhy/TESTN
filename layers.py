...
class SpatialEvoConv(nn.Layer):
    def __init__(self, in_feats, out_feats, dist_dim, num_neighbor, time_list, boundaries, dist_embed, feat_drop, activation=None, hop1_fc=False, merge='sum'):
        ...
        self.time_2hop=MultiHeadGATLayer_timehop(in_feats, out_feats, dist_dim, boundaries, dist_embed, feat_drop, activation, hop1_fc)
        ...
        self.fc_time = [nn.Linear(out_feats, out_feats, weight_attr=w_fcw1, bias_attr=False) for i in range(4)]
    ...
    def forward(self, graph, feat):
        ...
        for i in range(len(self.time_list)):
            f_list[i]=f_list[i]+feat_list[i]

        output_list=[]
        for i, time in enumerate(self.time_list):
            output_list.append(self.fc_time[i](self.time_2hop(graph[self.time_list[(i+1)%4]],f_list[(i+1)%4],f_list[i])+self.time_2hop(graph[self.time_list[(i-1)%4]],f_list[(i-1)%4],f_list[i]))+f_list[i])
        return output_list

class MultiHeadGATLayer_twohop(nn.Layer):
    def __init__(self, in_feats, out_feats, dist_dim, boundaries, dist_embed, feat_drop, activation=None, hop1_fc=False, merge="cat",num_heads=4):
        super(MultiHeadGATLayer_twohop, self).__init__()
        self.heads = nn.LayerList()
        for i in range(num_heads):
            self.heads.append(TwoHopConv(in_feats, out_feats, dist_dim, boundaries, dist_embed, feat_drop, activation, hop1_fc))
        self.merge = merge
        self.attn_merge = nn.Linear(num_heads * out_feats, out_feats, bias_attr=False)

    def forward(self,graph, feat):
        head_outs = [attn_head(graph, feat) for attn_head in self.heads]
        if self.merge == "cat":
            return self.attn_merge(paddle.concat(head_outs, axis=1))
        else:
            return paddle.mean(paddle.stack(head_outs),axis=0)
        
class MultiHeadGATLayer_timehop(nn.Layer):
    def __init__(self, in_feats, out_feats, dist_dim, boundaries, dist_embed, feat_drop, activation=None, hop1_fc=False, merge="mean",num_heads=4):
        super(MultiHeadGATLayer_timehop, self).__init__()
        self.heads = nn.LayerList()
        for i in range(num_heads):
            self.heads.append(heter_TwoHopConv(in_feats, out_feats, dist_dim, boundaries, dist_embed, feat_drop, activation, hop1_fc))
        self.merge = merge
        self.attn_merge = nn.Linear(num_heads * out_feats, out_feats, bias_attr=False)

    def forward(self,graph, feat,feat1):
        head_outs = [attn_head(graph, feat,feat1) for attn_head in self.heads]
        if self.merge == "cat":
...
class heter_TwoHopConv(nn.Layer):
    def __init__(self, in_feats, out_feats, dist_dim, boundaries, dist_embed, feat_drop, activation=None, hop1_fc=False):
        ...
        w_dir = paddle.ParamAttr(initializer=nn.initializer.XavierUniform())
        ...
        self.dir_fc = nn.Linear(self.dir_embed, self.dir_embed, weight_attr=w_dir, bias_attr=False)
        self.attn_fc = nn.Linear(2 * out_feats, 1, bias_attr=False)
    ...
    def send_func(self, src_feat, dst_feat, edge_feat):
        ...
        spa_embed = paddle.concat([dst_feat['des_h'], dist_embed], axis=1)
        rel_embed = paddle.concat([dst_feat['h'], src_feat['h']], axis=1)
        spa_rel_v = self.attn_fc(paddle.concat([self.fc_w1(spa_embed),self.fc_w2(rel_embed)], axis=1))
        alpha = self.leaky_relu(spa_rel_v)
    ...
    def forward(self, graph, feat,feat1):
        ...
        node_feat = {'h': h_2hop, 'loc': graph.node_feat['loc'], 'h1':feat1}
        msg = graph.send(self.send_func,
                            src_feat={'loc': node_feat['loc'],  'h': node_feat['h']},
                            dst_feat={'loc': node_feat['loc'],  'h': node_feat['h'],
                                      'des_h': node_feat['h1']},
                            edge_feat=None)
...
class TwoHopConv(nn.Layer):
    ...
    def send_func(self, src_feat, dst_feat, edge_feat):
        ...
        return {'alpha': alpha, 'h': src_feat['h_2hop']}
...
class SpatialEvoProp(nn.Layer):
    ...
    def send_func(self, src_feat, dst_feat, edge_feat):
        ...
        h_dist = dst_feat['d0'] * src_feat['d2'] * self.agg_fc(paddle.concat([dist_embed1, src_feat['h']], axis=1))
        return {'h': h_dist}
